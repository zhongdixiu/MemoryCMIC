...
| 2026-09-21 | 根据 Honcho 实库验证修正来源关系；补充迁移后画像重建要求及“通用迁移内核 + 来源适配器”方案 |
...
两个字段都是检索时的硬性适用范围，不是来源标签或内容主题分类。当前验证样例使用的业务域语义如下：

| 业务域 | 适用场景 | 不应仅因以下线索就归入 |
| --- | --- | --- |
| `email` | 邮件的撰写、总结、发送、回复及管理 | 只是出现“发送”，或来源为 `email_agent` |
| `disk` | 云盘或文件存储系统中的整理、上传下载、权限及链接分享 | 只是提到文件或路径，或来源为 `disk_agent` |
| `communication` | 电话、短信、即时消息等非邮件通讯操作 | 只是一般性提到“沟通”；邮件在当前扁平词表中单独归入 `email` |

这些值是当前 V1.0 验证词表，不是数据库枚举。写入服务应使用受控词表校验，避免为同一业务产生多种拼写。

标注时遵循以下规则：

- `source_system` 只记录来源系统；不得因来源是 `email_agent` 或 `disk_agent` 就自动写入对应业务域；
- 非空范围必须由记忆命题本身，或随来源一起保存的显式、可审计上下文支持；
- 如果在其他业务中使用该记忆仍然正确，则 `business_domains` 为 `NULL`；仅当跨业务使用会导致错误行为时才限定业务域；
- 项目名出现在内容中不必然意味着项目限制；只有记忆命题仅在特定项目内成立时，才写入 `project_domains`；
- 证据来自哪些业务或项目，不等于推断结论的适用范围；范围必须与推断结论的文本语义一致；
- `NULL` 表示已确认不受该维度限制，不表示“尚未判定”。V1.0 不表达未知范围；无法根据命题或显式上下文判定时，应补充上下文或暂缓形成正式记忆。

典型标注如下：

| 记忆命题 | `business_domains` | `project_domains` | 原因 |
| --- | --- | --- | --- |
| project_a 的邮件总结使用项目符号 | `{email}` | `{project_a}` | 同时限定邮件业务和具体项目 |
| 所有项目邮件都使用简洁草稿 | `{email}` | `NULL` | 限定邮件业务，但不限定具体项目 |
| 呼叫前从通讯录确认号码 | `{communication}` | `NULL` | 只限通讯操作，与项目无关 |
| project_c 的材料目录路径 | `NULL` | `{project_c}` | 邮件等业务也可用该路径查找附件，但事实只属于 project_c |
| 云盘文件对外分享使用批准链接 | `{disk}` | `NULL` | 规则明确限于云盘业务，对所有项目生效 |
...
Honcho 是首个完成实库验证的来源适配器。本章先规定 Honcho 的已确认映射，再定义适用于
OpenClaw 等后续来源的通用迁移边界。
...
| `documents.internal_metadata.message_ids` | `memory_evidence` supports | 经实库验证后按 `messages.id` 解析 |
| `documents.source_ids` | `memory_evidence` derives | 经实库验证后按上游 `documents.id` 解析 |
...
- `internal_metadata.message_ids` 和 `source_ids` 缺失、为空、无法解析的数量；
- 已软删除 document 数量；
...
- 迁移脚本可重复执行，不生成重复数据；
- 任一来源字段无法解析且缺少完整依据时建立 `legacy_import` 来源，不伪造原 message 或
  Document 关系；
- 已删除 document 映射为 `invalidated`，不直接丢弃；
- 首次只迁移代表性小批次，通过后再全量迁移；
- 全部向量在目标系统重新生成。
- 正式迁移导入用户记忆后必须触发画像重建，不直接把非结构化 Document 冒充为画像属性。
...
flowchart TD
    ...
    E --> F["创建 vector_upsert 任务"]
    E --> H["按用户聚合创建 profile_rebuild 任务"]
    F --> G["一致性校验与迁移报告"]
    H --> G
```
...
5. 读取 document 的 `internal_metadata.message_ids`，通过 message 映射找到目标
   `source_record`，建立 supports；读取 `source_ids`，通过 document 映射找到上游
   `memory_item`，建立 derives；
6. 为来源、上游记忆与下游记忆创建 `memory_evidence`；
7. 为新增或发生有效变化的记忆创建 `vector_upsert`；按受影响用户聚合创建
   `profile_rebuild`，不得为每条记忆重复创建画像任务；
...
### 12.5 Honcho 来源字段与证据组

当前已验证 Honcho 实例存在两类来源字段：

- `internal_metadata.message_ids` 使用数值 ID 指向 `messages.id`，迁移为 supports；
- `source_ids` 使用 Document ID 指向上游 `documents.id`，迁移为 derives。

来源适配器必须依据实际 schema 和数据分布确认字段语义，不能仅凭字段名推断。多个 message
或上游 Document 可能共同支撑结论，不得默认解释为彼此独立的 OR 证据。

V1.0 采用保守映射：

- 同一个 Document 的全部已确认依据默认放入同一个 `evidence_group_id`；
...
- 没有可解析依据的 Document 建立一条 `source_type=legacy_import` 的合成来源，并在元数据中
  保留 Honcho Document ID 和无法匹配的原始 ID；
- 引用部分缺失时保留可解析的 supports/derives，同时使用明确标记的 `legacy_import` 承接
  不完整部分，不伪造不存在的 message 或 Document。
...
- 迁入 `subject_type=user` 的 active 事实或推断后，正式迁移程序必须按用户聚合创建
  `profile_rebuild`，不能只创建向量任务；
- `profile_rebuild` 使用 `target_type=subject`，payload 至少携带 `subject_type`、`subject_id`、
  来源系统、namespace 和本批记忆水位或稳定摘要；任务继承迁移批次 `correlation_id`，
  批量迁移使用 `priority=0`；
- 画像重建幂等键必须包含租户、用户、来源空间和稳定水位。相同数据重复迁移不得重复创建任务，
  后续新增或变更记忆则必须形成新的重建水位；
- 画像 Worker 只使用当时仍有效的事实和推断生成 `profile_property`，并通过 `profile_basis`
  关联依据 memory；不得生成无依据画像；
- `peer_card` 等非结构化画像候选数据应先形成可追溯 memory。除非来源适配器已有经过确认的
  `property_key`、类型、值和冲突规则，否则不得直接写入 `profile_property`；
...
画像计算依赖核心服务中的属性归一化、冲突处理和 `profile_rebuild` Worker。在这些能力尚未实现时，
迁移验证只能确认来源、记忆、证据、映射、审计和任务契约，不能声称已完成画像生成或画像质量
验收。验证报告必须把画像重建标记为 deferred；核心服务完成后再执行端到端补充验收。

### 12.7 通用迁移内核与来源适配器

Honcho、OpenClaw 及其他 Agent 或系统的数据结构和记忆语义不同。统一记忆服务采用“通用迁移
内核 + 来源适配器”，不为每个来源复制目标库事务、幂等、审计和对账实现。

| 组件 | 职责 |
| --- | --- |
| 通用迁移内核 | 执行目标写入、确定性 ID、`external_entity_mapping`、事务、审计、任务派发、幂等和对账 |
| 来源适配器 | 只读读取源数据，解释身份、范围、状态、版本、删除语义和来源关系，生成规范化迁移计划 |
| 画像重建 Worker | 消费 `profile_rebuild`，从迁入 memory 计算结构化画像并建立 `profile_basis` |

来源适配器统一输出以下规范化迁移计划，不直接操作目标业务表：
...
evidence_links
subject_mappings
profile_rebuild_subjects
unresolved_items
```

通用迁移内核只接受通过校验的计划，并负责：

1. 在同一目标事务中写入业务实体、外部映射和审计；
2. 使用 `source_system + source_namespace + entity_type + external_id` 追溯外部实体；
3. 生成 `vector_upsert`、`profile_rebuild` 等正常运行期任务，不创建来源专属 task type；
4. 保证同一计划重复执行新增为 0，并对 mapping 存在但目标缺失等严重错误立即停止；
5. 输出来源数量、迁移数量、跳过、缺失关系、无法识别主体、任务和第二次执行新增数。

每个来源适配器至少必须确认：

- 数据读取方式、实际 schema 或导出格式及只读保证；
- 外部账号、用户、Agent、company 到 `subject` 和 author 的映射；
- 外部 message、document、memory block、文件等实体如何映射为 source 或 memory；
- 来源引用是 supports、derives 还是无法解释；
- 删除、版本、时间、TTL、范围和冲突语义；
- 现有画像字段是先作为 memory 导入，还是具备直接映射结构化属性的充分条件；
- 缺失关系、未知类型和重复记录的停止或降级策略。

`external_entity_mapping.entity_type` 当前枚举以 Honcho 实体为基线。新来源实体只有在语义真实一致时
才可归一为 message 或 document；如果 OpenClaw 等来源存在无法对应的 `memory_file`、
`memory_block` 等实体，应通过版本化数据库 migration 扩展枚举，不得为绕过约束伪装成 Honcho
实体。

### 12.8 新来源迁移的确认边界

通用架构可在没有具体来源数据时冻结，但某一来源的迁移可行性只有在取得真实 schema 或脱敏样例
后才能确认。OpenClaw 等新来源必须依次完成：

1. Source Profile 和字段/关系分布统计；
2. 主体、来源、记忆、证据和状态语义确认；
3. dry-run 与无法映射项报告；
4. 代表性小批量实际迁移；
5. 重复执行、悬空证据、审计、任务和源库只读对账；
6. 核心画像能力可用后，补充 `profile_rebuild -> profile_property + profile_basis` 端到端验收。

在第1～2步完成前，只能确认“通用迁移内核 + 来源适配器”的方案方向，不能确认该来源的具体
字段映射、数据完整性或迁移可行性。
...
4. 来源更新、删除、TTL 和依赖传播；
5. 画像投影、推断形成和重建；
6. Honcho 全量迁移、向量重建和用户画像重建；
7. 按“通用迁移内核 + 来源适配器”接入 Email Agent、OpenClaw 等外部记忆；
8. 管理后台和审计展示；
9. 性能、安全与生产化增强。
...
- 以通用外部实体映射表、通用迁移内核和来源适配器支撑外部记忆的可追溯、可重跑迁移；
- 以迁移后的 `profile_rebuild` 将用户记忆投影为具备 `profile_basis` 的结构化画像。
